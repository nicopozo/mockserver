	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != (tt.wantedErr != nil) { //nolint
				t.Errorf("SearchResponseForMethodAndPath() error = %v, wantedErr %v", err, tt.wantedErr != nil) //nolint
				return
			}
			...
			if !reflect.DeepEqual(tt.want, got) { //nolint
				t.Errorf("Response is not the expected. Expected: %v - Actual: %v", tt.want, got) //nolint
			}
		})
	}
}
